...
function Addon:GetWelcomeController()
	...
	if not controller then
		controller = LibChev.NewWelcomeController({
			addonName = "NoPoizen",
			linkType = "nopoizenfeedback",
			curseforgeURL = "https://www.curseforge.com/wow/addons/nopoizen",
			githubURL = "https://github.com/AlexAllocated/NoPoizen",
			command = "/np",
			clients = "Retail, WoW Forever, Classic Era, Hardcore, Season of Discovery, TBC Anniversary, Mists Classic, and Titan Reforged",
			getVersion = function()
				return self.API.GetAddOnVersion(self.addonName)
			end,

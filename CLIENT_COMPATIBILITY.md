...
- Era/Hardcore/SoD and TBC use per-weapon temporary enchants. The exact client catalog controls rank and charge handling. SoD-specific Sebacious, Numbing, Atrophic and Occult poisons are in the Era catalog only. Equipped Deadly Brew counts as automatic poison coverage; learning its engraving recipe alone does not. The detector reads `C_Engraving.GetRuneForEquipmentSlot(5).itemEnchantmentID` after checking whether engraving is enabled, and fails quietly if the rune observation is unreadable. TBC has its additional ranks and Anesthetic Poison, with no charge requirement.
...
- Era `SpellEffect` spell `400080` applies chest rune enchant `6708`, which grants `399969`, `399965`, `433521`. Their `Spell` descriptions establish automatic Instant Poison on unpoisoned weapons. The equipped-rune API is documented in `Blizzard_APIDocumentationGenerated/EngravingInfoDocumentation.lua` in the pinned Era source.
